...
#'
#' @param beta Selected SNP effect size estimates
#' @param se_beta Selected SNP effect size standard error
...
#'
#' @return list with elements: rb_beta, rb_se if `est_se_beta` = TRUE and only a vector of Rao-Blackwellized estimates if `est_se_beta` = FALSE
#' @export
#'
#' @examples
#' alpha <- 5e-5
...
snp_beta_rb <- function(
  ...
  est_se_beta = TRUE
    ...
    rb_beta <- beta - (se_beta / eta) * ratio_term
    ...
    if (!est_se_beta) {
      ...
      return(list(rb_beta = rb_beta, rb_se = sqrt(rb_var)))
    }
}
