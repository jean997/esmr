...
```{r}
grapple_dat <- data.frame(cbind(sim_dat1$beta_hat[, c(1, 3, 4)], 
                                sim_dat1$se_beta_hat[, c(1, 3, 4)]))
names(grapple_dat) <- c("gamma_exp1", "gamma_exp2", "gamma_out1", 
                        "se_exp1", "se_exp2", "se_out1")

grapple_dat <- mutate(grapple_dat, 
                      pval_1 = 2*pnorm(-abs(gamma_exp1/se_exp1)),
                      pval_2 = 2*pnorm(-abs(gamma_exp2/se_exp2)))
grapple_dat$selection_pvals <- with(grapple_dat, pmin(pval_1, pval_2))
...
grapple_res <- data.frame(est = res$beta.hat, 
                          ...
                          truth = c(sim_dat1$total_trait_effects[1,4], sim_dat1$direct_trait_effects[3,4]))
grapple_res
...
Fit multivariable MR adjusting for $X$, $Y$, and $Z$. What are the true effects that we are estimating doing this? How does the linear regression method compare to GRAPPLE?
