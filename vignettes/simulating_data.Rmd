...
The function can be used to generate data with LD by inputting a list of LD matrices and a corresponding table of SNP information. The function will work fastest if the LD matrix is broken into small blocks. For this example we will use the LD pattern estimated from Chromosome 19 in HapMap3 which can be downloaded (here). The function currently requires you to input a list of eigen decomposition generated by `eigen` applied to each block in the LD matrix. This is the format of the linked file. 

Download the LD data and put them in a directory called `data`. Let's look at the LD data
